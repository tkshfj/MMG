...
# # add this helper (right after _pick_logits_from_output, for example)
# def decision_from_logits(
#     logits: torch.Tensor,
#     threshold: float,
#     *,
...
# # Decision policy (single SoT for logits->pos_prob + threshold)
# def init_decision_policy(
#     evaluator: Engine,
#     *,
#     decision_cfg: Mapping[str, Any] | None = None,
# ) -> None:
#     """
#     Initializes evaluator.state with:
#       - posprob: PosProbCfg(...) (single source of truth for logits->pos_prob)
#       - to_pos_prob: callable(logits)->probs
#       - threshold: float decision threshold (calibrator can overwrite later)
#     """
...
#     )
#     evaluator.state.posprob = pp

#     # unified callable for any consumer (metrics, watchdogs, search)
#     def _to_pos_prob(logits: torch.Tensor) -> torch.Tensor:
#         return pp.logits_to_pos_prob(logits)

#     evaluator.state.to_pos_prob = _to_pos_prob  # function
#     evaluator.state.threshold = float(cfg.get("cls_threshold", 0.5))


# def get_active_threshold(evaluator: Engine, default: float = 0.5) -> float:
#     thr = getattr(evaluator.state, "threshold", None)
#     return float(thr if thr is not None else default)
...
#     non_blocking: Optional[bool] = None,
#     trainer_for_logging: Optional[Engine] = None,
#     decision_cfg: Mapping[str, Any] | None = None,   # NEW
# ) -> Engine:
#     """Custom evaluator that returns an out_map dict compatible with metrics."""
#     nb_default = bool(non_blocking) if non_blocking is not None else False
...
#         try:
#             logits = extract_cls_logits_from_any(net_out)
#             logits = logits.float() if torch.is_tensor(logits) else torch.as_tensor(logits, dtype=torch.float32, device=dev)
#             out_map["y_pred"] = logits          # logits only; never probs here
#             out_map["logits"] = logits
#             out_map["cls_out"] = logits
#             if y is not None:
#                 out_map["y"] = torch.as_tensor(y, device=dev).long().view(-1)
...
#     # Persist device hints on the engine (spawn-safe)
#     setattr(evaluator.state, "device", device)
#     setattr(evaluator.state, "non_blocking", bool(getattr(data_loader, "pin_memory", False)))

#     # NEW: install single SoT policy (pos_prob + threshold)
#     init_decision_policy(evaluator, decision_cfg=decision_cfg)
