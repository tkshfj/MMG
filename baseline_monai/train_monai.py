...
class DenseNetWithDropout(nn.Module):
    def __init__(self, dropout_rate=0.2, pretrained=False):
        super().__init__()
        ...
        # Add dropout after the penultimate layer (after features, before classifier)
        # Use getattr for compatibility across MONAI versions
...
def compute_pos_weight(train_loader):
    # Computes the ratio of negatives to positives for pos_weight in BCEWithLogitsLoss
    total, pos = 0, 0
...
def main():
    ...
    state_dict = torch.load("./models/model_final.pth", map_location=DEVICE)
