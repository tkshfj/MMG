...
print("Training completed.")
    ...
    # val_epoch_loss = 0
    # val_class_preds, val_class_targets = [], []
    # val_dice_scores_manual = []  # For optional manual dice
    ...
    #         # Debug/Visualization: Only on epoch 0
    #         if epoch == 0:
    #             print(f"Val seg out stats: min={seg_out.min().item():.3f}, max={seg_out.max().item():.3f}, mean={seg_out.mean().item():.3f}")
    #             print(f"Val mask stats: min={masks.min().item()}, max={masks.max().item()}, mean={masks.mean().item():.3f}")
    #             pred_probs = torch.sigmoid(seg_out)
    #             print(f"Val pred mask mean: {pred_probs.mean().item():.3f}, min: {pred_probs.min().item():.3f}, max: {pred_probs.max().item():.3f}")
    #             pred_bin = (pred_probs > 0.5).float()
    #             print(f"Val pred binary unique: {torch.unique(pred_bin)}")
    ...
    #         union = pred_bin.sum().item() + masks.sum().item()
    #         dice_manual = (2. * intersection) / (union + 1e-8)
    #         val_dice_scores_manual.append(dice_manual)

    # val_avg_loss = val_epoch_loss / len(val_loader)
    # val_dice_score = dice_metric.aggregate().item()
    # val_dice_score_manual = np.mean(val_dice_scores_manual)
    # # print(f"[DEBUG] DiceMetric not_nans: {dice_metric.not_nans}")
    # # print(f"[DEBUG] DiceMetric dice_sum: {dice_metric.get_buffer('dice_sum')}")
    # print(f"[DEBUG] DiceMetric reduction: {dice_metric.reduction}")
    # print(f"[DEBUG] DiceMetric include_background: {dice_metric.include_background}")
    # dice_metric.reset()
    # val_acc = np.mean(np.array(val_class_preds) == np.array(val_class_targets))
    # try:
    #     val_rocauc = roc_auc_score(val_class_targets, val_class_preds)
    ...
    # print(f"[Epoch {epoch}] Validation Dice (MONAI): {val_dice_score:.4f}, Manual Dice: {val_dice_score_manual:.4f}")
    # print(
    #     f"Epoch {epoch} | "
    ...
    #     "train_dice_manual": train_dice_score_manual,
    ...
    #     "val_dice_manual": val_dice_score_manual,
    #     "val_sens": val_sens,
    #     "val_spec": val_spec,
    #     "epoch": epoch,
    # })
